...
function fetchTemplates() {
    ...
    fetch('/api/templates')
        ...
        .then(data => {
            ...
            data.forEach(t => {
                html += `<tr><td><strong>${escapeHtml(t.name)}</strong></td><td>${escapeHtml(t.keys.join(', '))}</td><td style="text-align:right;"><button class="btn-red" onclick="deleteTemplate('${escapeHtml(t.name)}')" style="padding:4px 8px;">Delete</button></td></tr>`;
            });
            html += '</tbody></table>';
            el.innerHTML = html;
        });
