...
function rotateUserKey(uuid, user) {
    ...
    fetch('/api/user/rotate?token=' + window.authToken, { method: 'POST', body: formData })
        .then(response => {
            if (response.ok) {
...
function deleteUser(uuid, user) {
    ...
    formData.append('uuid', uuid);
    formData.append('user', user);
    fetch('/api/user/delete?token=' + window.authToken, { method: 'POST', body: formData })
        .then(response => {
...
function fetchTemplates() {
    ...
    fetch('/api/templates?token=' + window.authToken)
        .then(res => res.json())
        .then(data => {
            if (data.length === 0) { el.innerText = "No templates found."; return; }
            var html = '<table class="table"><thead><tr><th>Name</th><th>Keys</th><th>Actions</th></tr></thead><tbody>';
            data.forEach(t => {
                html += `<tr><td><strong>${t.name}</strong></td><td>${t.keys.join(', ')}</td><td style="text-align:right;"><button class="btn-red" onclick="deleteTemplate('${t.name}')" style="padding:4px 8px;">Delete</button></td></tr>`;
            });
            html += '</tbody></table>';
            el.innerHTML = html;
        });
...
function deleteTemplate(name) {
    ...
    fetch('/api/templates/' + name + '?token=' + window.authToken, { method: 'DELETE' }).then(r => fetchTemplates());
}
...
function fetchHistory() {
    ...
    fetch('/api/history?token=' + window.authToken)
        .then(res => res.json())
        .then(data => {
            var html = '';
            if (data.length === 0) html = '<tr><td colspan="5">No history.</td></tr>';
                ...
                data.forEach(e => {
                    html += `<tr><td>${e.ts.replace('T', ' ').replace('Z', '')}</td><td>${e.user}</td><td><strong>${e.action}</strong></td><td>${e.target}</td><td>${e.details}</td></tr>`;
                });
            }
            el.innerHTML = html;
        });
...
function openInfoModal(uuid) {
    ...
    if (data.host_keys && data.host_keys.length > 0) {
        data.host_keys.forEach(k => {
            tbody.innerHTML += `<tr><td>${k.type}</td><td style="font-family:monospace; word-break:break-all;">${k.key}</td></tr>`;
        });
...
function prepareConnect(uuid, user) {
    ...
    aliases.forEach((alias, index) => {
        ...
        container.innerHTML += `<div style="margin-bottom:8px;"><label style="display:flex;align-items:center;cursor:pointer;"><input type="radio" name="connect_host" value="${alias}" ${checked} style="margin-right:8px;"> ${alias}</label></div>`;
    });
...
function triggerConnect(user, host) {
    ...
    fetch('/connect?token=' + window.authToken, { method: 'POST', body: formData })
        .then(response => {
