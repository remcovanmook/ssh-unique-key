 ...
 *
 * Using a data island instead of inline `window.*` assignments avoids IDE
 * false-positive parse errors caused by Jinja2 double-brace syntax being
 * interpreted as JavaScript object literals.
 *
 * Populates:
 *   - window.identitiesList  {Array}   - list of identity objects from the server
 *   - window.xtermEnabled    {boolean} - whether the xterm terminal is available
 */
function initFromDataIsland() {
    ...
    try {
        ...
        window.identitiesList = data.identities || [];
        window.xtermEnabled   = !!data.xtermEnabled;
    } catch (e) {
