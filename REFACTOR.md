...
> checks pass. Vendored assets are pinned at @xterm/xterm@5.5.0 +
> @xterm/addon-fit@0.10.0 via `scripts/vendor-update` (dual-CDN
> cross-check). One note: task 6's regression test is a plain-bash
> `tests/bash/test_*.sh` (always runs, even without bats); the bats suite
> proper arrives in Phase 5.
